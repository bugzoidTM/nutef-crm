...
export interface ModeloDeFuncionario {
  ...
  /** Pacotes do motor que descrevem a função (rótulo na vitrine). */
  pacotes: readonly ToolBundle[];
  /**
   * As capacidades concretas que ele liga ao nascer — CURADAS, dentro do teto de
   * 25 por agente do motor (TETO_TOOLS_POR_AGENTE). Ligar os pacotes inteiros
   * passava de 38 e o botão "Criar" ficava travado (medido pela tela). Só
   * nomes do catálogo; o preenchimento descarta o que não tiver handler.
   */
